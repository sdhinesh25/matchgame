...
class App extends Component {
  ...
  imageChange = thumbnailUrl => {
    ...
    const getImage = imagesList.filter(
      eachList => eachList.thumbnailUrl === thumbnailUrl,
    )

    const {imageUrl} = getImage[0]
    if (imageUrl === imgUrl) {
      const newImg =
        imagesList[Math.floor(Math.random() * imagesList.length)].imageUrl
      this.setState(prevState => ({score: prevState.score + 1, imgUrl: newImg}))
    } else {
      clearInterval(this.timerId)
      this.setState({isTrue: true})
    }
  ...
  render() {
    ...
    const thumbnailList = imagesList.filter(
      eachList => eachList.category === category,
    )
        ...
        <div className="main-container">
          {!isTrue && (
            <div className="game-container">
              ...
              <ul className="category-part">
                {tabsList.map(eachList => (
                  <li key={eachList.tabId}>
                    <button
                      type="button"
                      className={`button ${
                        category === eachList.tabId ? 'highlight-button' : ''
                      }`}
                      onClick={() => this.changeCategory(eachList.tabId)}
                    >
              ...
              <ul className="multiple-img-container">
                {thumbnailList.map(eachList => (
                  <li key={eachList.id}>
                    <button
                      type="button"
                      className="thumbnail-img-btn"
                      onClick={() => this.imageChange(eachList.thumbnailUrl)}
                    >
                      <img
          ...
          {isTrue && (
            <div className="result-container">
              ...
              <p className="your-scores">{score}</p>
              <button
                type="button"
